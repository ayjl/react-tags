...
var ReactTags = React.createClass({
  propTypes: {
    ...
    exactMatch: React.PropTypes.bool,
    allowDuplicateTags: React.PropTypes.bool,
    stopSuggestionClickPropagation: React.PropTypes.bool,
    suggestionFilterType: React.PropTypes.oneOf(['startsWith', 'contains']),
    requireFocusToShowSuggestions: React.PropTypes.bool
  },
  ...
  filteredSuggestions(query, props) {
    var matches;
    ...
    return props.suggestions.filter(function(item) {
      return matches(item.toLowerCase().indexOf(query.toLowerCase()))
        && (props.allowDuplicateTags
            || props.tags.map(tag => tag.text).indexOf(item) === -1 )
    }.bind(this));
  },
  ...
  handleSuggestionMouseDown: function(i, e) {
    e.preventDefault();
    ...
    const tagInput =  !this.props.readOnly ? (
      <div className={this.state.classNames.tagInput}>
        ...
        <Suggestions query={query}
          ...
          requireFocusToShowSuggestions={this.props.requireFocusToShowSuggestions}
          />
      </div>
    ) : null;
